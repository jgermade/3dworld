// Patched in w3d (vendor/truck-shapeops/PATCHED.md): ordered containers, so the
// chain starts at the same vertex on every target. `FxHash` iteration order
// depends on the width of `usize`.
