...
| **Sharding across wasm32 heaps, not one wasm64 heap** | The interactive document lives in one shared memory with a rayon pool over it. Bulk work — large booleans, mass tessellation, a huge STEP import — goes to workers with their *own* memory, each with its own 4 GB. Total addressable exceeds the ceiling without paying for 64-bit pointers anywhere. Exchange is by transferable `ArrayBuffer`, which is a move. **What is in that buffer was hand-waved here until 2026-09-15 and is now specified in [WIRE.md](WIRE.md)**: one header and up to four sections, carrying a chunk of one body, divided by face because that is the axis the work divides on. |
...
| Browser build | `wasm-bindgen` 0.2.127 and a matching `wasm-bindgen-cli`, then `wasm-opt -O3` from binaryen `version_124` (`make binaryen`). On 2026-09-26, rustc 1.98.1: `make web` → 3.36 MiB of wasm, 1.26 MiB gzipped, 0.91 MiB brotli; `make web-threaded` → 3.38 MiB into `web/dist/threaded/`. A first single-threaded visit fetches **1.31 MiB gzipped** in all, a threaded one 2.62 MiB, because its worker still loads the single build — `make web-test` weighs what the page actually fetched. |
| Cross-architecture check | `make xarch` — one `measurements()` in `w3d-xarch`, built as a native binary and as a `wasm32-unknown-unknown` module, compared by `to_bits()`. Since 2026-09-26 every row is asserted: the operands, their bounds, their topology and their meshes at four sags; the cut's topology, its mesh, and every `f64` in the saved cut as a multiset; and each build deterministic across two processes. That rests on `truck-shapeops` being patched (`vendor/truck-shapeops/PATCHED.md`) — without it the six cut rows fail. Four negative controls run first. No `wasm-bindgen-cli`, no browser: a wasm32 target and a node. |
| Browser check | `make web-test` — Chromium via Playwright, five runs: WebGPU offered, WebGL2 forced, **the worker boundary** — a second wasm instantiation with its own linear memory, posting a tessellation as transferable buffers, which is the sharding row above actually happening — no COOP/COEP and no service worker, and no COOP/COEP with the service worker. The first and last also assert the variant, the pool size, and — when the threaded bundle is absent — that the page says so. |
