...
| Cross-architecture check | `make xarch` — one `measurements()` in `w3d-xarch`, built as a native binary and as a `wasm32-unknown-unknown` module, compared by `to_bits()`. It asserts what is **true today**: the operands, their bounds, their topology and their meshes at four sags are bit-identical on both, the topology of the cut is too, and each build is deterministic across two processes. It does **not** assert that the cut agrees, because it does not — see register item 4 — so those rows are held to 1% and reported. Five negative controls run first. No `wasm-bindgen-cli`, no browser: a wasm32 target and a node. |
