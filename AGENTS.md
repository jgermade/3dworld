...
- **A control that offers an operation asks whether the build has it.** `GeometryKernel::supports`
  takes a `Capability`, the conformance suite holds every backend to its answers in both
  directions, and a button, menu item or handle that a backend cannot serve is disabled with
  `Capability::absence` as its reason — not offered and then apologised to. Two things this is
  *not*. It is not a substitute for handling the refusal: a capability is a property of the build,
  answerable with no arguments, so `true` never promises the next call succeeds and the status bar
  still reports what a solid was refused for. And it is not a place to name an operation loosely —
  a capability is named for **the case that is actually declined**, so a backend that sweeps
  straight but not bent gives `BentSweep`, and a control doing the part that works stays enabled.
  Greying out a control that works is the same failure as offering one that does not, arriving by
  way of the fix.
...
| `w3d_kernel::conformance` | One suite in two halves, against *every* backend, `FakeKernel` included. The **contract** half — handles, bounds, well-formed meshes, promised errors, and the rule that a face id names one *connected* region of the surface — is run against all of them. The **geometry** half — the volume a mesh encloses, and what a boolean does to it — is run only where `does_geometry()` says the backend does any, because a bounding box satisfies every bounds assertion a boolean can be held to and one backend passed this suite for nine days doing exactly that. A backend may *decline* an operation and still conform; it may not answer wrongly, and three box fixtures are mandatory so that declining everything cannot pass. `kernel-fake`'s negative control holds the fake to the half it is excused from and requires it to fail. The geometry half both **weighs** a result and **compares its boundary**: a golden census sums each face into the plane it lies in — never per face, because whether a kernel splits a coplanar region into one face or two is its own business and both are correct — and asserts the area there, its centre of area, and the total area lying in no named plane. The centroid is the half a volume cannot reach: a hole drilled in the wrong place weighs exactly what a hole in the right place weighs. The census carries its own negative controls, run against a mesh built by hand with no kernel in it, one of which moves a hole and must be caught with every area unchanged. Since 2026-09-18 it also **probes every `Capability`** and requires the backend's answer and its behaviour to agree in both directions: a `true` that answers `Unsupported` is a lie, and so is a `false` the operation then performs — without the second half a backend passes by denying everything. Only `Unsupported` counts as the contradiction, because a probe that comes back `Degenerate` or `Failed` was *attempted*, which is what `true` claimed. `kernel-fake` carries the negative control, a backend with exactly one answer inverted, and requires the capability check to be the **only** failure. |
| `w3d_format` round-trip | A saved document keeps its nodes, names, visibility, tolerance, quality, shared bodies, **its assembly tree, the identity of every node in it and the unit it is in** — and refuses a file written by another kernel, both directions. Since version 2 the refusals are most of the file: a parent that is not earlier in the list, an identity that is not unique or not there, a counter behind an identity in the file, a unit nobody knows. Each is asserted to fail *as itself*, against a fixture asserted to load unaltered, because a negative control that fails for the wrong reason is the way this suite would rot. A **version-1** file — written out by hand, since the writer that produced it no longer exists — must still open, flat and unitless, and two saves of one document must be the same bytes. The container is asserted to be a zip a standard tool can open. |
...
| `make step-check` | STEP against something that is not us, in both directions: a file this program wrote, read by a pure-Python part-21 parser that shares no code with OCCT — every reference resolved, faces counted by surface type, so *the hole in the plate is asserted to be in the file* — and files from Pro/ENGINEER, Siemens NX and STEP Tools imported through the real reader, one of which must be refused because it is a surface model. It also weighs the one sample whose *arrangement* is known — the AS1 assembly's five parts placed eighteen times, ten assemblies, three levels deep, no two placements of a part sharing a centre — because a count cannot see a walk that flattens a tree or one that drops a placement's location. Since 2026-09-14 it also takes that assembly **through a `.w3d` and back** — eighteen bodies in ten assemblies, three levels deep, with real OpenCASCADE geometry under every leaf — and asserts the tree, the identities, the manifest's byte-for-byte reproducibility and each solid's bounds to a tolerance, which is the shape a four-node fixture in `format/tests/` cannot have. Needs `make step-samples` first, which fetches them pinned by SHA-256 and commits nothing. **What it does not do is prove another program can open ours**; nothing that could is installable, and the register says so. |
...
| `make web-scene` | Not a check, and it carries one. Writes `web/scene.w3d` — the document the page boots on, a `.w3d` the browser fetches and hands to its worker without ever opening it — and then **reads it back with a kernel that never saw the geometry and meshes it**, which is what `make step-check` does at the other end of this repository for the same reason: a writer that cannot demonstrate a reader is a writer nobody has checked. It refuses to write a file whose boolean declined, because a scene file quietly holding an uncut plate is how the browser spent nine days drawing one. Gitignored — a generated artifact in the tree can be older than its generator and still look authoritative — and `make web` depends on it. |
...
| `make web` | Not a check, but it carries one. Builds `web/dist/` — needs `wasm-bindgen-cli` at the `wasm-bindgen` dependency's version; a mismatch is a runtime error about an unknown import, not a build failure — and runs `wasm-opt -O3` from the binaryen release `make binaryen` fetches and hash-checks, because the distribution's is too old to parse the module. `tools/wasm_payload.py` then weighs the artifact raw, gzipped and brotli'd, and **fails if it still carries a `name` section**, which is what a module that skipped `wasm-opt` looks like. `WASM_OPT=none` builds unoptimised on purpose and says so. |
...
| `make measure` | Not a check, and the only place a number about *this* program's performance comes from this program. What an assembly costs on the thread that asked, phase by phase: STEP import, a mesh per body, the 28-byte packing a worker would post, a `.w3d` written — **twice**, because a document that has been drawn does not weigh what one that has not weighs — and opened again. It asserts nothing: there is no golden number for a duration, and a busy machine would fail one. Release only; the example refuses to print a debug timing, because a debug timing is a number somebody quotes six months later. Needs OCCT and `make step-samples`. |
...
- **An import answers with the file's tree, or with no tree, and never with a broken one.**
  `Import { bodies, assemblies }` — a body per solid *placement*, and the assembly nodes they sat
  in, which carry no geometry and so are a list of their own. A backend whose reader sees no
  structure says so with no assemblies and no parents, which is what a flat file deserves.
  `Import::validate` is the whole of the rule: every index in range, and every assembly's parent
  *earlier in the list* than itself, so a cycle is unrepresentable rather than detectable and one
  pass can build the tree. Two placements of one part are two independent bodies; the seam has no
  instance of a `Body` and does not pretend to.
...
- **A node in a file has an identity, and it is the only thing that survives being written
  down.** `uid` is what a `parent` refers to; a name is a label two nodes may share and a position
  in a list moves. The rule the reader enforces is the same one, for the same reason, that
  `Import::validate` carries at the seam: **a parent appears earlier in the list than its
  children**, which makes a cycle unrepresentable rather than detectable and lets a file be built
  in one pass with no visited set and no recursion. The counter that hands identities out is saved
  with the document, because a reader resuming from the largest identity *present* would give a
  deleted node's identity to the next node created.
